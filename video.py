...
def _parse_params(chunks):
    # supports values that might include '=' by splitting only once
    params = {}
    for s in chunks:
        if '=' in s:
            k, v = s.split('=', 1)
...
def create_capture(source=0, fallback=presets['chess']):
    ...
    if cap is None or not cap.isOpened():
        print(f'Warning: unable to open video source: {source0}')
...
if __name__ == '__main__':
    ...
    # map() is an iterator in Python 3, so we must materialize it once
    caps = [create_capture(s) for s in sources]
    ...
    while True:
        ...
        if ch == ord(' '):
            for i, img in enumerate(imgs):
                fn = f'{shotdir}/shot_{i}_{shot_idx:03d}.bmp'
                cv2.imwrite(fn, img)
                print(fn, 'saved')
            shot_idx += 1
